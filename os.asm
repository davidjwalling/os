...
;       device, class, sub-class, prog IF and revision. Controller behavior might differ based on revision. We check
...
;       field contents have not changed, but the cursor has moved, the loop re-enters at .1500. When neither the field
...
;       Routine:        ConPCIProbe
;
;       Description:    This routine handles the lspci command.
;
;-----------------------------------------------------------------------------------------------------------------------
...
;       Routine:        ConDiskInfo
;
;       Description:    This routine displays disk info
;
;-----------------------------------------------------------------------------------------------------------------------
ConDiskInfo             push    ebx                                             ;save non-volatile regs
                        push    esi                                             ;
                        push    edi                                             ;

                        mov     edx,czNetInfoTitle                              ;string address
                        putConsoleString                                        ;show string

                        mov     edx,01F6h
                        mov     al,0A0h                                         ;master drive
                        out     dx,al                                           ;primary bus drive select

                        mov     edx,01F2h
                        xor     al,al                                           ;zero
                        out     dx,al                                           ;set sector count to zero
                        inc     dx
                        out     dx,al                                           ;set LBA-lo to zero
                        inc     dx
                        out     dx,al                                           ;set LBA-mid to zero
                        inc     dx
                        out     dx,al                                           ;set LBA-high to zero

                        mov     edx,01F7h
                        mov     al,0ECh                                         ;identify command
                        out     dx,al                                           ;send to command i/o port
                        in      al,dx                                           ;read command i/o port

                        mov     edx,czNotFound
                        test    al,al                                           ;test if drive exists
                        jz      .50
                        mov     edx,czFound
                        putConsoleString

                        mov     ecx,01FFFFFh                                    ;long wait
.10                     mov     edx,01F7h                                       ;status port
                        in      al,dx                                           ;read status
                        test    al,080h                                         ;test busy bit
                        loopnz  .10                                             ;wait
                        ...
                        mov     ecx,01FFFFFh                                    ;long wait
.20                     mov     edx,01F7h                                       ;status
                        in      al,dx
                        test    al,01h                                          ;error
                        jnz     .30                                             ;yes, continue
                        test    al,08h                                          ;sector buffer requires servicing?
                        loopz   .20
...
.25                     mov     edx,01F0h                                       ;data port
                        ...
                        mov     edx,czReadComplete
                        jmp     short .50


.30                     mov     edx,czDiskError
                        jmp     short .50
.40                     mov     edx,czNonATA
.50                     putConsoleString

                        pop     edi                                             ;restore non-volatile register
                        pop     esi                                             ;
                        pop     ebx                                             ;
                        ret                                                     ;return
;-----------------------------------------------------------------------------------------------------------------------
...
;       check if token is d
;
.50                     cmp     al,EASCIILOWERD                                 ;disk info?
                        jne     .90                                             ;no, branch
                        lodsb                                                   ;next token character
                        test    al,al                                           ;end of token
                        jnz     .90                                             ;no, branch
...
;       Display network info
;
                        call    ConDiskInfo                                     ;display disk information
;
;       Restore and return.
;
...
;
;-----------------------------------------------------------------------------------------------------------------------
ConRead                 push    ebx                                             ;save non-volatile regs

                        mov     edx,01F6h                                       ;drive and head port
                        mov     al,0A0h                                         ;drive 0, head 0
                        out     dx,al                                           ;select drive head

                        mov     edx,01F2h                                       ;sector count port
                        mov     al,1                                            ;read one sector
                        out     dx,al                                           ;set sector count

                        mov     edx,01F3h                                       ;sector number port
                        mov     al,1                                            ;sector 1
                        out     dx,al                                           ;set sector number

                        mov     edx,01F4h                                       ;cylinder low port
                        mov     ax,0                                            ;cylinder low/high
                        out     dx,ax                                           ;select cylinder

                        mov     edx,01F7h                                       ;command port
                        mov     al,020h                                         ;read with retry
                        out     dx,al                                           ;send command

                        mov     ecx,01FFFFh
.10                     in      al,dx                                           ;read status
                        test    al,8                                            ;sector requires service?
                        loopz   .10                                             ;keep trying
...
czTitle                 db      13,10,"Operating System Version 0.X"            ;title and copyright
                        db      13,10,"Copyright 2010 David J. Walling. MIT License.",0
...
czDisplayOptions        db      13,10,"d           display command options"
                        db      13,10,"d d         display disk information"
...
czAM79C970              db      "PCnet-PCI II Am79C970",0                       ;PCnet-PCI II AM79C970/70C971
                                                                                ...
                                                                                ;       Disk information
                                                                                ;---------------------------------------
czNetInfoTitle          db      13,10,"Primary Bus Master ATA ",0
czFound                 db      "exists",0
czNotFound              db      "not found",0
czBusyTimeout           db      13,10,"ATA busy timeout",0
czReadyTimeout          db      13,10,"ATA ready timeout",0
czReadComplete          db      13,10,"ATA read complete",0
czDiskError             db      13,10,"ATA disk error",0
czNonATA                db      13,10,"ATA non-ATA",0
