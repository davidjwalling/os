...
tsvc			tsvce	PutConsoleString				;tty output asciiz string
			...
			tsvce	LeftTrimString					;left-trim string
			tsvce	RightTrimString					;right-trim string
			tsvce	UpperCaseString					;upper-case string
			tsvce	CompareStrings					;compare strings
...
%macro			placeCursor 0
			...
			int	_svc						;invoke OS service
%endmacro
%macro			leftTrimString 0
			mov	al,eLeftTrimString				;al = left trim fn.
			int	_svc						;invoke OS service
%endmacro
%macro			rightTrimString 0
			mov	al,eRightTrimString				;AL = right trim fn.
			int	_svc						;invoke OS service
%endmacro
%macro			upperCaseString 0
			...
			int	_svc						;invoke OS service
%endmacro
%macro			compareStrings 0
			mov	al,eCompareStrings				;AL = compare strings fn.
			int	_svc						;invoke OS service
%endmacro
%macro			resetSystem 0
...
;-----------------------------------------------------------------------------------------------------------------------
;
;	Routine:	LeftTrimString
;
;	Description:	Trim leading blanks from a given string.
;
;	In:		DX:EBX	string address
;
;	Out:		EAX	string address
;
;-----------------------------------------------------------------------------------------------------------------------
LeftTrimString		push	esi						;save volatile regs
			push	edi						;
			push	es						;
			push	ds						;store data selector ...
			pop	es						;... in extra segment
			mov	esi,ebx						;string address
			mov	edi,ebx						;string address
			cld							;forward strings
.10			lodsb							;string char
			cmp	al,EASCIISPACE					;space?
			je	.10						;yes, repeat
			dec	esi						;non-space char address
.20			lodsb							;non-space char
			stosb							;store in output
			or	al,al						;null?
			jnz	.20						;repeat until done
			mov	eax,ebx						;string address
...
;-----------------------------------------------------------------------------------------------------------------------
;
;	Routine:	RightTrimString
;
;	Description:	Trim trailing blanks from a given string.
;
;	In:		DS:EBX	string address
;
;	Out:		EAX	string address
;
;-----------------------------------------------------------------------------------------------------------------------
RightTrimString		push	edi						;save volatile regs
			push	es						;
			push	ds						;store data selector ...
			pop	es						;... in extra segment
			call	GetStringLength					;string length in eax
			mov	ecx,eax						;string length
			jecxz	.10						;skip ahead if no length
			mov	edi,ebx						;string address
			add	edi,ecx						;null address
			dec	edi						;last char address
			std							;backward strings
			mov	al,EASCIISPACE					;ascii space comparator
			repe	scasb						;search backward for non-space
			cld							;forward strings
			mov	byte [edi+2],0					;null-terminate string
.10			mov	eax,ebx						;string address
			pop	es						;restore volatile regs
			pop	edi						;
			ret							;return to caller
;-----------------------------------------------------------------------------------------------------------------------
;
;	Routine:	GetStringLength
;
;	Description:	Return count of characters in string.
;
;	In:		DS:EBX	string address
;
...
;
;-----------------------------------------------------------------------------------------------------------------------
GetStringLength		push	ecx						;save volatile regs
			push	esi						;
			xor	eax,eax						;zero length
			mov	ecx,ebx						;string address
			jecxz	.20						;exit if no address
			mov	esi,ebx						;string address
			cld							;forward strings
.10			lodsb							;string char
			or	al,al						;null?
			jnz	.10						;no, repeat
			mov	eax,esi						;after-null address
			dec	eax						;null address
			sub	eax,ecx						;length of string
.20			pop	esi						;restore volatile regs
			pop	ecx						;
			ret							;return to caller
;-----------------------------------------------------------------------------------------------------------------------
;
...
;
;-----------------------------------------------------------------------------------------------------------------------
UpperCaseString		push	esi						;save volatile regs
			mov	esi,ebx						;string address
.10			lodsb							;string character
			...
			jmp	.10						;continue
.20			mov	eax,ebx						;string address
			pop	esi						;restore volatile regs
			ret							;return to caller
;-----------------------------------------------------------------------------------------------------------------------
;
...
;
;	In:		DS:EBX	string 1 address
;			DS:EDX	string 2 address
;
;	Out:		EAX	0	strings are equal
;				<0	string 1 < string 2
;				>0	string 1 > string 2
;
;-----------------------------------------------------------------------------------------------------------------------
CompareStrings		push	ecx						;save volatile regs
			push	esi						;
			push	edi						;
			push	es						;
			push	ds						;store data selector ...
			pop	es						;... into es register
			mov	edi,edx						;string 2 address
			mov	esi,ebx						;string 1 address
			xor	eax,eax						;prime result
.10			mov	cl,[esi]					;string 1 char
			cmpsb							;compare char
			jne	.20						;exit if not equal
			or	cl,cl						;end of strings?
			jnz	.10						;no, repeat
			jmp	.30						;yes, exit
.20			inc	eax						;+1 : assume string 1 > string 2
			jnc	.30						;exit if true
			neg	eax						;-1 : string 1 < string 2
.30			pop	es						;restore volatile regs
			pop	edi						;
			pop	esi						;
			pop	ecx						;
			ret							;return to caller
...
;
;	Description:	Update the shift/ctrl/alt/lock indicators in the operator information area (OIA).
;
;	Input:		bl	shift flags
;			bh	lock flags
;			ds	OS data selector
;
;-----------------------------------------------------------------------------------------------------------------------
PutConsoleOIAShift	push	ecx						;save volatile regs
			push	es						;
			push	ESELCGA						;load CGA selector ...
			pop	es						;... into ES register
...
;
;	Description:	Update the Operator Information Area (OIA).
;
;	In:		ds	OS data selector
;
;-----------------------------------------------------------------------------------------------------------------------
PutConsoleOIAChar	push	ebx						;save volatile regs
			push	ecx						;
			push	esi						;
			push	ds						;
			push	es						;
...
.30			add	bl,2						;next column (+2)
			...
			call	SetConsoleChar					;display ASCII character
.40			pop	es						;restore volatile regs
			pop	ds						;
			pop	esi						;
			pop	ecx						;
			pop	ebx						;
...
;
;	Description:	Write two ASCII characters to the console screen representing the value of a byte.
;
;	In:		al	byte value
;			ch	row
;			cl	column
...
;
;-----------------------------------------------------------------------------------------------------------------------
PutConsoleHexByte	push	ebx						;save volatile regs
...
;	Routine:	ResetSystem
;
;	Description:	Restart the system using the 8042 controller
;
;-----------------------------------------------------------------------------------------------------------------------
...
;
;	Description:	Send the set/reset mode indicators command.
;
...
;
;	Description:	Wait for keyboard input buffer to be ready for input.
;
;	Out:		ZF	1 = Input buffer ready
;				0 = Input buffer not ready after timeout
;
;-----------------------------------------------------------------------------------------------------------------------
WaitForKeyInBuffer	push	ecx						;save volatile regs
...
;
;	Description:	Wait for keyboard output buffer to have data to read.
;
;	Out:		ZF	0 = Output buffer has data from controller
;				1 = Output buffer empty after timeout
;
;-----------------------------------------------------------------------------------------------------------------------
WaitForKeyOutBuffer	push	ecx						;save volatile regs
			mov	ecx,EKEYWAITLOOP				;keyboard controller timeout
.10			in	al,EKEYPORTSTAT					;keyboard status byte
...
.10			putConsoleString czPrompt				;display input prompt
			...
			mov	ebx,wzConsoleInBuffer				;input buffer address
			call	ConParseCommand					;parse the command line
...
;-----------------------------------------------------------------------------------------------------------------------
;
;	Routine:	ConParseCommand
;
;	Description:	Parse console command
;
;	input:		ds:ebx	command address
;
;	output:		eax	command nbr
;
;-----------------------------------------------------------------------------------------------------------------------
ConParseCommand		push	esi						;save volatile regs
			push	edi						;
			leftTrimString						;left-trim string at ebx
			rightTrimString						;right-trim string at ebx
			upperCaseString						;upper-case string at ebx
			xor	edi,edi						;command nbr
			mov	esi,tConCmdTbl					;commands table
.10			cld							;forward strings
			lodsb							;command length (including null)
			or	al,al						;end of table?
			jz	.20						;yes, exit
			movzx	ecx,al						;length
			push	ecx						;save length
			mov	edx,esi						;command
			compareStrings						;compare strings at ebx,edx
			pop	ecx						;restore length
			or	eax,eax						;equal?
			jz	.20						;yes, exit
			inc	edi						;increment command nbr
			add	esi,ecx						;point to next command
			jmp	.10						;repeat
.20			mov	eax,edi						;command number
			pop	edi						;restore volatile regs
			pop	esi						;
			ret							;return to caller
;-----------------------------------------------------------------------------------------------------------------------
;
;	Routine:	ConClearCommand
;
;	Description:	Handle the clear command
;
;-----------------------------------------------------------------------------------------------------------------------
ConClearCommand		clearConsoleScreen					;clear console screen
			ret							;return to caller
;-----------------------------------------------------------------------------------------------------------------------
;
;	Routine:	ConExit
;
;	Description:	Handle the exit  command
;
;-----------------------------------------------------------------------------------------------------------------------
...
;	Routine:	ConVersion
;
;	Description:	Handle the ver (version) command
;
;-----------------------------------------------------------------------------------------------------------------------
...
tConJmpTbl		equ	$						;command jump table
			dd	ConClearCommand - ConCode			;clear command routine offset
...
tConCmdTbl		equ	$						;command name table
			db	6,"CLEAR",0					;clear screen command
			db	5,"EXIT",0					;exit command
			db	4,"VER",0					;version command
			db	0						;end of table
;-----------------------------------------------------------------------------------------------------------------------
;
;	Constants
